...
import { useAdminDeleteDraftSupplierOrder } from '@/lib/hooks/api/draft-sorders';
...
const Information = ({ dorder, isLoading }: Props) => {
	const cancelOrder = useAdminDeleteDraftSupplierOrder(dorder?.id!);
...
const OrderStatus = ({ status }: { status: DraftSupplierOrder['status'] }) => {
	switch (status) {
		case 'open':
			return (
				<StatusIndicator
