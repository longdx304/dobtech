...
import {
	useAdminCompleteDraftSupplierOrder,
	useAdminDeleteDraftSupplierOrder,
	useAdminDraftSupplierOrders,
} from '@/lib/hooks/api/draft-sorders';
...
import draftOrderColumns from './draft-sorder-column';

type Props = {};

const DEFAULT_PAGE_SIZE = 10;

const SupplierDraftOrderList: FC<Props> = () => {
	...
	// Mutations api
	const completeOrder = useAdminCompleteDraftSupplierOrder(
		currentDraftOrderId ?? ''
	);
	const cancelOrder = useAdminDeleteDraftSupplierOrder(
		currentDraftOrderId ?? ''
	);
	...
	const { draft_supplier_orders, isLoading, count } =
		useAdminDraftSupplierOrders(
			{
	...
	}, [draft_supplier_orders]);

	const handleRowClick = (record: any) => {
		router.push(`${ERoutes.SUPPLIER_DRAFT_ORDERS}/${record.id}`);
	};
	...
	return (
		<div className="w-full">
			...
			<Table
				...
				dataSource={draft_supplier_orders ?? []}
				rowKey="id"
				scroll={{ x: 700 }}
				onRow={(record) => ({
...
export default SupplierDraftOrderList;
