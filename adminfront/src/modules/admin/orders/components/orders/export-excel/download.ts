...
/**
 * Downloads Excel files for the given order data
...
export const downloadExcelFiles = async (excelFiles: ExcelFile[]) => {
	try {
		...
		excelFiles.forEach((file) => {
			...
			// Generate filename
			const filename = `Order_${file.displayId}_${file.soChungTu || 'export'}.xlsx`;
...
export const downloadCSVFiles = (excelFiles: ExcelFile[]) => {
	excelFiles.forEach((file) => {
		...
		const filename = `Order_${file.displayId}_${file.soChungTu || 'export'}.csv`;
