...
type Props = {
	...
	handleCancel: () => void;
	regionId: string;
	returnShippingOption?: ShippingOption;
};

const ReturnShippingOptionModal: FC<Props> = ({
	state,
	handleOk,
	handleCancel,
	regionId,
	...
	const createReturnShippingOption = useAdminCreateShippingOption();
	const updateReturnShippingOption = useAdminUpdateShippingOption(
	...
	const isCreate = _.isEmpty(returnShippingOption);
	...
	const onFinish = async (values: AdminPostShippingOptionsReq) => {
		...
		if (isCreate) {
			await createReturnShippingOption.mutateAsync(
				{
					...payload,
					is_return: true,
				},
				{
					onSuccess: () => {
						message.success('Tạo mới phương thức giao hàng trả lại thành công');
						form.resetFields();
						handleOk();
					},
					onError: (error: any) => {
		...
		if (!isCreate) {
			...
			await updateReturnShippingOption.mutateAsync(updatedPayload as any, {
				onSuccess: () => {
					message.success('Cập nhật phương thức giao hàng trả lại thành công');
					handleOk();
				},
				onError: (error: any) => {
					message.error(getErrorMessage(error));
	...
	useEffect(() => {
		if (!isCreate && returnShippingOption) {
			const requirements = getRequirementsData(returnShippingOption as any);
			...
			form.setFieldsValue({
				...
				price_type: returnShippingOption?.price_type,
				amount: returnShippingOption?.amount,
				store_option: !returnShippingOption?.admin_only,
				shipping_profile: returnShippingOption?.profile?.name,
	...
	return (
		<SubmitModal
			title={
				isCreate
					? 'Tạo phương thức Giao Hàng Trả Lại'
					: 'Cập nhật phương thức Giao Hàng Trả Lại'
			}
			open={state}
			onOk={handleOk}
			...
			form={form}
			loading={
				createReturnShippingOption.isLoading ||
				updateReturnShippingOption.isLoading
			}
			width={800}
		>
...
export default ReturnShippingOptionModal;
